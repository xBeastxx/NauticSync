                                ...
                                >
                                    ...
                                    <button
                                        onClick={async (e) => {
                                            ...
                                            setIsMenuOpen(false);
                                            try {
                                                const result = await window.electronAPI.importGitignore(folder.path);
                                                if (result.imported > 0) {
                                                    alert(`Imported ${result.imported} patterns from .gitignore (${result.total} total in .stignore)`);
                                                } else {
                                                    alert('No .gitignore found or no patterns to import.');
                                                }
                                            } catch (err) {
                                                console.error(err);
                                                alert('Failed to import .gitignore');
                                            }
                                        }}
                                        className="w-full text-left px-3 py-2 text-xs text-zinc-400 hover:bg-zinc-800 hover:text-white flex items-center gap-2"
                                    >
